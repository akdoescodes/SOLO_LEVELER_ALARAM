...
export function ContextMenu({ visible, onClose, buttonPosition, onDelete }: ContextMenuProps) {
  ...
  // Calculate consistent positioning for both web and mobile
  const menuTop = buttonPosition.y + (Platform.OS === 'web' ? 35 : 45); // Adjust for platform differences
  const menuRight = 16; // Always 16px from screen edge
  const menuWidth = Math.min(160, SCREEN_WIDTH - buttonPosition.x - 32); // Ensure it fits
